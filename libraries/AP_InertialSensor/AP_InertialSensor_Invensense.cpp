...
AP_InertialSensor_Invensense::AP_InertialSensor_Invensense(AP_InertialSensor &imu,
    ...
    : AP_InertialSensor_Backend(imu), _temp_filter(1000, 1), _rotation(rotation), _dev(std::move(dev))
{
}
...
{
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "starting init");
    ...
    if (success)
    {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "MPU6500: Hardware initialization successful");
    }
    else
    {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "MPU6500: Hardware initialization failed");
    }
    return success;
}
...
void AP_InertialSensor_Invensense::_fifo_reset(bool log_error)
{
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "MPU6500: FIFO reset performed");
}
...
bool AP_InertialSensor_Invensense::_check_whoami(void)
{
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "MPU WHOAMI: 0x%02x", whoami);
    switch (whoami)
    {
...
bool AP_InertialSensor_Invensense::_hardware_init(void)
{
    ...
    if (!_check_whoami())
    {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "MPU6500: WHOAMI check failed");
        return false;
    }

    // Debug print statement after WHOAMI check
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "MPU6500: WHOAMI check successful");
    // Chip reset
    uint8_t tries;
    ...
    if (tries == 5)
    {
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_CRITICAL, "MPU6500: failed to boot 5 times");
        return false;
    }
...
AP_Invensense_AuxiliaryBusSlave::AP_Invensense_AuxiliaryBusSlave(AuxiliaryBus &bus, uint8_t addr,
                                                                 uint8_t instance)
    : AuxiliaryBusSlave(bus, addr, instance), _mpu_addr(MPUREG_I2C_SLV0_ADDR + _instance * 3), _mpu_reg(_mpu_addr + 1), _mpu_ctrl(_mpu_addr + 2), _mpu_do(MPUREG_I2C_SLV0_DO + _instance)
{
}
